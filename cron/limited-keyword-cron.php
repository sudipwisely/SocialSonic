...
$AutoSQL = mysql_query("SELECT * FROM " . DB_PREFIX . "prospects` as `pro` LEFT JOIN " . DB_PREFIX . "customers` as `cus` ON `pro`.`Cust_ID` = `cus`.`Cust_ID` WHERE (`cus`.`Cust_Payment_Type` = 'trip') GROUP BY `pro`.`Cust_ID` ORDER BY `pro`.`Prospect_ID` DESC");
if ( $AutoSQL ) {
